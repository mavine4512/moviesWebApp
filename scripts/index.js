...
function initialMovieList() {
  getMovies("Vikings").then(({ movies = [], totalResult = 0 }) => {
...
function initListeners() {
  ...
  searchInput.addEventListener("click", searchInMovies);
...
function searchInMovies(e) {
  ...
  SEARCH_DEBOUNCE_FLAG = setTimeout(() => {
    let trend = movieNameRef.value;
    if (!trend) trend = "Vikings";
  ...
  movieElm.innerHTML = `
            ...
            <div class="movie-item__meta">
                ...
                <span class="movie-item__meta__imdb-link">
                    ...
                    <button class="movie-item__detail-button">View Details</button>
            </div>`;
